...
(function() {
  ...
  function plotData(map) {
    ...
    d3.selectAll('#allFilters').on('change', update);

    function update() {
      gSelected = d3.select('#generation').property('value')
      lSelected = d3.select('#legendary').property('value')
     ...
     if (gSelected == 'all' && lSelected == 'all') {
        ...
        dots
        .filter(function(d) {return ((gSelected != d['Generation']) && (lSelected != d['Legendary']));})
        .attr("display", 'none');
    ...
    //     dots
    //     .filter(function(d) {return ((lSelected != d['Legendary']) && (gSelected != d['Generation']));})
    //     .attr("display", displayOthers);
  ...
  function drawAxes(limits, x, y) {
    ...
    let yScale = d3.scaleLinear()
      .domain([limits.yMax + 5, limits.yMin - 50]) // give domain buffer
      .range([50, 500]);
